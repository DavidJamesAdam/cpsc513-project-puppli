...
async def create_user(user: User) -> User:
    ...
    try:
        ...
        await run_in_threadpool(
            _finalize_txn, txn2, user_ref, email, displayName, city, province
        )
        ...
        raise HTTPException(
            ...
            detail=f"Error writing profile: {e}",
        )
...
def read_users():
    ...
    try:
        ...
        users = [UserInfo.model_validate({**doc.to_dict()}) for doc in docs]
...
async def get_user(user_id: str, user=Depends(auth_check)):
    ...
    try:
        ...
        target_doc = db.collection("users").document(target_user_id).get()
        if not target_doc.exists:
            raise HTTPException(status_code=404, detail="User not found")

        doc = target_doc.to_dict()
        if not doc or doc.get("deletedAt"):
            raise HTTPException(status_code=404, detail="User not found")
...
async def update_user(
    user_id: str, updated_fields: UpdateUser, user=Depends(auth_check)
):
    """
    Update user info.
    ...
    try:
        ...
        await require_owner_or_admin(owner_id=user["uid"], user=user)
...
async def delete_user(user_id: str):
    try:
        ...
        await run_in_threadpool(
            user_ref.update, {"deletedAt": datetime.now(timezone.utc).isoformat()}
        )
