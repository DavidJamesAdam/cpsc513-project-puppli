...
def generate_post(user_id: str, pet_id: str, pet_name: str, city_name: str,
                  province_name: str):
...
def seed_database():
  ...
  for _ in range(NUM_USERS):
    ...
    # don't read back from Firestore here (batch.set not yet committed)
    ...
    for _ in range(pet_count):
      ...
      for _ in range(post_count):
        post_data = generate_post(
            ...
            pet_ref.id,
            pet_data["name"],
            firestore_user["cityName"],
            firestore_user["provinceName"],
        )
...
def delete_collection(collection_name: str, batch_size: int = 500):
  ...
  deleted = 0
  # Only delete Firebase Authentication users when removing the `users` collection
  delete_auth_users = collection_name == "users"
  ...
  while True:
    ...
    for doc in docs:
      # queue deletion from Firestore
      batch.delete(doc.reference)

      # if this is the users collection, also remove the user from Firebase Auth
      if delete_auth_users:
        try:
          auth.delete_user(doc.id)
        except auth.UserNotFoundError:
          # user already deleted in Auth; continue
          pass
        except Exception as e:
          print(f"Error deleting auth user {doc.id}: {e}")
